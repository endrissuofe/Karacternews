...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_articles_status" AS ENUM('draft', 'in_review', 'scheduled', 'published', 'archived');
  CREATE TYPE "public"."enum_users_role" AS ENUM('admin', 'editor', 'author', 'contributor');
  CREATE TYPE "public"."enum_redirects_to_type" AS ENUM('reference', 'custom');
  CREATE TYPE "public"."enum_forms_confirmation_type" AS ENUM('message', 'redirect');
  CREATE TYPE "public"."enum_payload_jobs_log_task_slug" AS ENUM('inline', 'publishScheduledArticles', 'schedulePublish');
  CREATE TYPE "public"."enum_payload_jobs_log_state" AS ENUM('failed', 'succeeded');
  CREATE TYPE "public"."enum_payload_jobs_task_slug" AS ENUM('inline', 'publishScheduledArticles', 'schedulePublish');
  ...
  CREATE TABLE "podcast_audio" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"duration" numeric,
  	"prefix" varchar DEFAULT 'podcast-audio',
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"url" varchar,
  	"thumbnail_u_r_l" varchar,
  ...
  CREATE TABLE "site_settings" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"site_name" varchar DEFAULT 'Karacter News' NOT NULL,
  	"logo_id" integer,
  	"social_links_x" varchar,
  	"social_links_instagram" varchar,
  	"social_links_linkedin" varchar,
  	"youtube_channel_url" varchar,
  	"youtube_live_stream_url" varchar,
  	"homepage_featured_hero_id" integer,
  	"updated_at" timestamp(3) with time zone,
  	"created_at" timestamp(3) with time zone
  );
  ...
  ALTER TABLE "site_settings" ADD CONSTRAINT "site_settings_homepage_featured_hero_id_articles_id_fk" FOREIGN KEY ("homepage_featured_hero_id") REFERENCES "public"."articles"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "site_settings_homepage_featured_homepage_featured_hero_idx" ON "site_settings" USING btree ("homepage_featured_hero_id");
  ...
  // Full-text search (Increment 3): weighted generated tsvector + GIN index.
  // Hand-written SQL — deliberately invisible to Payload/Drizzle snapshots,
  // so it must be re-added whenever migrations are consolidated.
  await db.execute(sql`
